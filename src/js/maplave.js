export default class maplave extends Phaser.Scene {
  constructor() {
    super({ key: 'maplave' });
  }
  ...
  create(data) {
    const map = this.make.tilemap({ key: 'carte_lave' });
    ...
    this.sound.play('pirate', { loop: false });
  ...
  update() {
    if (Phaser.Input.Keyboard.JustDown(this.toucheP)) {
      this.registry.set('resumeKey', 'maplave');
      this.scene.pause('maplave');
